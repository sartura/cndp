...
icmp6_output_header(pktmbuf_t *m, uint16_t nxt)
{
    ...
    if (!md)
        return ICMP6_OUTPUT_NEXT_PKT_DROP;
    ...
    icmp6 = pktmbuf_adjust(m, struct icmp6_hdr *, -len);
    if (!icmp6)
        return ICMP6_OUTPUT_NEXT_PKT_DROP;
...
static uint16_t
icmp6_output_node_do(struct cne_graph *graph, struct cne_node *node, void **objs, uint16_t nb_objs)
{
    ...
    next_index = ICMP6_OUTPUT_NEXT_IP6_OUTPUT;
    ...
    while (n_left_from >= 4) {
        ...
        next0 = icmp6_output_header(mbuf0, ICMP6_OUTPUT_NEXT_PKT_DROP);
        next1 = icmp6_output_header(mbuf1, ICMP6_OUTPUT_NEXT_PKT_DROP);
        next2 = icmp6_output_header(mbuf2, ICMP6_OUTPUT_NEXT_PKT_DROP);
        next3 = icmp6_output_header(mbuf3, ICMP6_OUTPUT_NEXT_PKT_DROP);
...
static uint16_t
icmp6_output_node_process(struct cne_graph *graph, struct cne_node *node, void **objs,
                          uint16_t nb_objs)
{
    return icmp6_output_node_do(graph, node, objs, nb_objs);
}

static struct cne_node_register icmp6_output_node_base = {
    .process = icmp6_output_node_process,
    .flags   = CNE_NODE_INPUT_F,
    .name    = ICMP6_OUTPUT_NODE_NAME,

    .nb_edges = ICMP6_OUTPUT_NEXT_MAX,
    .next_nodes =
        {
            [ICMP6_OUTPUT_NEXT_PKT_DROP]   = PKT_DROP_NODE_NAME,
            [ICMP6_OUTPUT_NEXT_IP6_OUTPUT] = IP6_OUTPUT_NODE_NAME,
        },
};

CNE_NODE_REGISTER(icmp6_output_node_base);
