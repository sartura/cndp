...
nd6_process_na(icmp6ip_t *iip)
{
    ...
    if (entry->reach_state == ND_INCOMPLETE) {
        ...
        while (quedPkts) {
            /* Now send any packets queued in entry->ar_packets_queue for the
             * neighbor awaiting address resolution.
             * Note:- This is not implemented yet. To compensate this we send
             *        such packets to linux kernel.
             */
        }
