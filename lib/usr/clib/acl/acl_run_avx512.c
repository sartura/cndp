...
#include "acl_run_sse.h"        // for search_sse_4, search_sse_8
#include "acl.h"                // for cne_acl_classify_scalar, cne_acl_ctx
#include "acl_run.h"            // for MAX_SEARCHES_AVX16, MAX_SEARCHES_SSE4
#include "cne_acl.h"            // for CNE_ACL_RESULTS_MULTIPLIER
#include "cne_vect.h"           // for xmm_t

/*sizeof(uint32_t) << match_log == sizeof(struct cne_acl_match_results)*/
...
static inline void
acl_set_flow_avx512(struct acl_flow_avx512 *flow, const struct cne_acl_ctx *ctx, uint32_t trie,
                    const uint8_t *data[], uint32_t *matches, uint32_t total_packets)
{
    flow->num_packets   = 0;
...
update_flow_mask(const struct acl_flow_avx512 *flow, uint32_t *fmsk, uint32_t *rmsk)
{
    uint32_t i, j, k, m, n;
 ...
 */
static inline void
resolve_mcle8_avx512x1(uint32_t result[], const struct cne_acl_match_results pr[],
                       const uint32_t match[], uint32_t nb_pkt, uint32_t nb_cat, uint32_t nb_trie)
{
    ...
    for (k = 0; k != nb_pkt; k++, result += nb_cat) {
        ...
        for (j = 0; j != nb_cat; j += CNE_ACL_RESULTS_MULTIPLIER) {
...
#include "acl_run_avx512x8.h"        // for search_avx512x8x2

int
cne_acl_classify_avx512x16(const struct cne_acl_ctx *ctx, const uint8_t **data, uint32_t *results,
...
#include "acl_run_avx512x16.h"        // for search_avx512x16x2

int
cne_acl_classify_avx512x32(const struct cne_acl_ctx *ctx, const uint8_t **data, uint32_t *results,
                           uint32_t num, uint32_t categories)
{
    const uint32_t max_iter = MAX_SEARCHES_AVX16 * MAX_SEARCHES_AVX16;
