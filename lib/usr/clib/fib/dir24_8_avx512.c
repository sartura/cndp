...
dir24_8_vec_lookup_x16(void *p, const uint32_t *ips, uint64_t *next_hops, int size)
{
    ...
    if (msk_ext != 0) {
        ...
        if (size == sizeof(uint8_t)) {
            idxes = _mm512_mask_i32gather_epi32(zero, msk_ext, idxes, (const int *)dp->tbl8, 1);
            idxes = _mm512_and_epi32(idxes, res_msk);
        } else if (size == sizeof(uint16_t)) {
            idxes = _mm512_mask_i32gather_epi32(zero, msk_ext, idxes, (const int *)dp->tbl8, 2);
            idxes = _mm512_and_epi32(idxes, res_msk);
        } else
            idxes = _mm512_mask_i32gather_epi32(zero, msk_ext, idxes, (const int *)dp->tbl8, 4);
...
dir24_8_vec_lookup_x8_8b(void *p, const uint32_t *ips, uint64_t *next_hops)
{
    ...
    if (msk_ext != 0) {
        ...
        idxes = _mm512_mask_i64gather_epi64(zero, msk_ext, idxes, (const void *)dp->tbl8, 8);
...
void
cne_dir24_8_vec_lookup_bulk_1b(void *p, const uint32_t *ips, uint64_t *next_hops,
                               const unsigned int n)
{
    uint32_t i;
    for (i = 0; i < (n / 16); i++)
        dir24_8_vec_lookup_x16(p, ips + i * 16, next_hops + i * 16, sizeof(uint8_t));

    dir24_8_lookup_bulk_1b(p, ips + i * 16, next_hops + i * 16, n - i * 16);
}

void
cne_dir24_8_vec_lookup_bulk_2b(void *p, const uint32_t *ips, uint64_t *next_hops,
                               const unsigned int n)
{
    uint32_t i;
    for (i = 0; i < (n / 16); i++)
        dir24_8_vec_lookup_x16(p, ips + i * 16, next_hops + i * 16, sizeof(uint16_t));

    dir24_8_lookup_bulk_2b(p, ips + i * 16, next_hops + i * 16, n - i * 16);
}

void
cne_dir24_8_vec_lookup_bulk_4b(void *p, const uint32_t *ips, uint64_t *next_hops,
                               const unsigned int n)
{
    uint32_t i;
    for (i = 0; i < (n / 16); i++)
        dir24_8_vec_lookup_x16(p, ips + i * 16, next_hops + i * 16, sizeof(uint32_t));

    dir24_8_lookup_bulk_4b(p, ips + i * 16, next_hops + i * 16, n - i * 16);
}

void
cne_dir24_8_vec_lookup_bulk_8b(void *p, const uint32_t *ips, uint64_t *next_hops,
                               const unsigned int n)
{
    uint32_t i;
    for (i = 0; i < (n / 8); i++)
