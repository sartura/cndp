...
#ifndef _CNE_HASH_CRC_H_
#define _CNE_HASH_CRC_H_
...
/**
 * Allow or disallow use of SSE4.2 intrinsics for CRC32 hash
 * calculation.
 *
 * @param alg
 ...
 */
static inline void
cne_hash_crc_set_alg(uint8_t alg)
{
...
// clang-format off
CNE_INIT(cne_hash_crc_init_alg)
{
 ...
 */
static inline uint32_t
cne_hash_crc(const void *data, uint32_t data_len, uint32_t init_val)
{
    unsigned i;
    uintptr_t pd = (uintptr_t)data;

    for (i = 0; i < data_len / 8; i++) {
        init_val = cne_hash_crc_8byte(*(const uint64_t *)pd, init_val);
        pd += 8;
    }

    if (data_len & 0x4) {
        init_val = cne_hash_crc_4byte(*(const uint32_t *)pd, init_val);
        pd += 4;
    }

    if (data_len & 0x2) {
        init_val = cne_hash_crc_2byte(*(const uint16_t *)pd, init_val);
        pd += 2;
    }

    if (data_len & 0x1)
        init_val = cne_hash_crc_1byte(*(const uint8_t *)pd, init_val);
